...
def add_rms_norm_pad(
    ...
    hidden_states = hidden_states.to(original_dtype) * weight
    return torch.nn.functional.pad(
        hidden_states, (0, padded_hidden_size - hidden_states.shape[-1])
    )


@torch.compile(backend=current_platform.simple_compile_backend)
def add_add_rms_norm(
    hidden_states: torch.Tensor,
    ...
    hidden_states = hidden_states.float() + (
        attention_output.float() + residual.float()
    )
...
def _materialize_attention_residual(
    attention_output: torch.Tensor, residual: torch.Tensor
) -> torch.Tensor:
    return (attention_output.float() + residual.float()).to(attention_output.dtype)
...
class MLPBlock(torch.nn.Module):
    def __init__(
        ...
        self.padded_hidden_size = self.experts.moe_config.hidden_dim
...
class TransformerBlock(torch.nn.Module):
    ...
    def forward(
        ...
        elif attention_output is None:
            hidden_states, residual = self.input_layernorm(hidden_states, residual)
        else:
            hidden_states, residual = add_add_rms_norm(
                hidden_states,
                attention_output,
                residual,
        ...
        attention_output = self.attn(hidden_states, positions)
        if (
            current_platform.is_cuda()
            and self.mlp.padded_hidden_size > self.mlp.hidden_size
        ):
            hidden_states = add_rms_norm_pad(
                attention_output,
                residual,
                self.post_attention_layernorm.weight,
                self.post_attention_layernorm.variance_epsilon,
                self.mlp.padded_hidden_size,
            )
            output = self.mlp(hidden_states)
            return output, residual, attention_output
...
class GptOssModel(nn.Module, EagleModelMixin):
    ...
    def forward(
        ...
        for i in range(self.start_layer, self.end_layer):
            layer = self.layers[i]
            x, residual, attention_output = layer(
                x, positions, residual, attention_output
            )
            if i + 1 in self.aux_hidden_state_layers and attention_output is not None:
                layer_residual = _materialize_attention_residual(
                    attention_output, residual
                )
            else:
                layer_residual = residual
            self._maybe_add_hidden_state(
                aux_hidden_states, i + 1, x, layer_residual
            )
        if not get_pp_group().is_last_rank:
            if attention_output is not None:
                residual = _materialize_attention_residual(attention_output, residual)
            return IntermediateTensors({"hidden_states": x, "residual": residual})
        if attention_output is None:
            x, _ = self.norm(x, residual)
        else:
            x, _ = add_add_rms_norm(
                x,
                attention_output,
                residual,
                self.norm.weight,
