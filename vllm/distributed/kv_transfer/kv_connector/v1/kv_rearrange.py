...
def rearrange_kernel_read(
    ...
    tp_group = curr_h * d // H
    dst_h = curr_h % (H // d)
    tp_group_offset = curr_n * (block_size //
                                d) + curr_b * (H // d) * C + dst_h * C + curr_c
...
def rearrange_tensors(t1: torch.Tensor, t2: torch.Tensor, d: int,
    ...
    assert t2.shape == (N, B, H,
                        C), "Destination tensor must have same shape as source"
    ...
    if direction == "read":
        ...
        rearrange_kernel_write[grid](t1,
                                     t2,
                                     N,
                                     B,
                                     H,
