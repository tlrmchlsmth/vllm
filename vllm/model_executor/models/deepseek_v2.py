...
class Indexer(nn.Module):
    ...
    def forward(
        ...
        # Apply RoPE in-place to the pe slices — layout is [pe, nope].
        # rotary_emb (FlashInfer/vLLM CUDA) modifies q/k in-place, so no
        # split+cat is needed: the original q and k already contain the
        # updated pe dims after this call.
        rotary_emb(
            positions,
            q[:, :, : self.rope_dim],
